...
function screenshot(url: string, width = 1500) {
...
export function SavanaClientCasePage({ slug }: { slug: string }) {
  ...
  const modules = [
    ...
    { title: 'Execução do case', text: project.delivery },
    ...
    <main className="case-page">
      <section className="section-frame case-hero portfolio-case-hero portfolio-case-hero-rich">
        <div className="case-hero-copy">
          <span className="eyebrow">Case Tehkné • {project.sector}</span>
          ...
          <div className="hero-actions">
            ...
            {project.externalOfficialUrl ? <a className="btn btn-secondary" href={project.externalOfficialUrl} target="_blank" rel="noreferrer">Abrir site oficial <ArrowUpRight size={16} /></a> : null}
          </div>
          <div className="case-stack">{project.stack.map((tech) => <span key={tech}>{tech}</span>)}</div>
        </div>
        <div className="case-visual">
          {project.externalOfficialUrl ? (
            <div className={`portfolio-case-preview portfolio-real-hero-preview ${project.tone}`}>
              ...
              <img src={screenshot(project.externalOfficialUrl)} alt={`Print do site oficial ${project.title}`} loading="eager" />
              <div className="portfolio-real-hero-caption"><span>Site oficial</span><strong>{project.title}</strong></div>
            </div>
          ) : (
      ...
      <section className="metrics section-frame compact">
        ...
        <div><strong>{project.externalOfficialUrl ? 'Site oficial' : 'Fonte pendente'}</strong><span>imagem de referência</span></div>
      ...
      <section className="section-frame case-reading-section-tight">
        <div className="section-heading">
          ...
          <p>Este case mostra repertório real em comunicação, campanha, design aplicado, WordPress, landing pages, conteúdo e sustentação para marcas de diferentes setores.</p>
        </div>
      ...
      {project.externalOfficialUrl ? <CaseScreens title={project.title} url={project.externalOfficialUrl} /> : null}
      ...
      <section className="section-frame os-section">
        <div className="os-panel">
          ...
          <h2>Experiência de comunicação convertida em capital técnico.</h2>
      ...
      <ContactForm page={`Case ${project.title}`} context={`case ${project.title}, ${project.category}, ${project.sector}, stack ${project.stack.join(', ')}`} title="Quer criar um case com esse padrão?" description="Use este bloco para chamar a Tehkné no WhatsApp com contexto de campanha, site, WordPress, branding ou presença digital." />
    </main>
  );
}
