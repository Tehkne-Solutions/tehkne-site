...
import { ArrowUpRight, BarChart3, CheckCircle2, Layers3, LayoutDashboard, Network, ShieldCheck, Sparkles, Workflow } from 'lucide-react';
...
const drEduardoModules: CaseModule[] = [
  {
    title: 'Contexto clínico',
    text: 'Marca profissional de saúde e odontologia com necessidade de transmitir confiança, clareza e autoridade local.'
  },
  {
    title: 'Arquitetura WordPress',
    text: 'Estrutura editável para organizar páginas, serviços, conteúdo institucional, SEO local e manutenção futura.'
  },
  {
    title: 'Experiência visual',
    text: 'Interface limpa, profissional e objetiva, com foco em leitura rápida e segurança para o paciente.'
  },
  {
    title: 'Resultado comercial',
    text: 'Vitrine de saúde com estética confiável, conteúdo organizado e caminho direto para contato e conversão.'
  }
];
...
const drEduardoProcess: Array<[string, string, string]> = [
...
export function PortfolioCasePage({ slug }: Props) {
  ...
  const process = isDrEduardo ? drEduardoProcess : defaultProcess;
    ...
    <main className="case-page">
      ...
      <section className="section-frame case-hero portfolio-case-hero portfolio-case-hero-rich">
        <div className="case-hero-copy">
          ...
          <h1>
            {project.title}:{' '}
            {isDrEduardo
              ? 'saúde e odontologia com autoridade clínica, agendamento premium e base WordPress validada.'
              : `${project.category.toLowerCase()} com participação técnica rastreável.`}
          </h1>
          <p>{project.summary}</p>
      ...
      <section className="section-frame case-reading-section-tight">
        <div className="section-heading">
          ...
          <p>
            {isDrEduardo
              ? 'Este case foi ajustado para mostrar a imagem real da base UNTI, cards mais específicos e stack corrigida por validação interna.'
              : 'Cada case apresenta repertório técnico, tipo de participação, stack, contexto e resultado esperado sem criar confusão de autoria.'}
          </p>
        </div>
        <div className="case-reading-grid case-reading-grid-balanced">
          {modules.map((module, index) => {
            ...
            return (
              <article className="glass-card case-reading-card case-reading-card-balanced" key={module.title}>
      ...
      {asset?.fullImage ? (
        <section className="section-frame case-full-showcase-section">
          <div className="section-heading">
            ...
            <h2>Visual real usado no ecossistema UNTI Digital.</h2>
            <p>Imagem completa do case, usada como evidência visual em vez de placeholder genérico.</p>
          </div>
          <div className="case-full-showcase">
            <div className="case-window-bar"><span /><span /><span /></div>
            <img src={asset.fullImage} alt={`Print completo do site ${project.title}`} loading="lazy" />
          </div>
        </section>
      ) : null}
      ...
      <section className="section-frame os-section">
        <div className="os-panel">
          ...
          <div className="os-features">
            {effectiveStack.map((item) => <span key={item}>{item}</span>)}
          </div>
        </div>
