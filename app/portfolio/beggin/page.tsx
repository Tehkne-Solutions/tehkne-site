import { ArrowUpRight, Award, BottleWine, Brush, Globe2, Leaf, ShoppingBag, Zap } from 'lucide-react';
  ...
  return (
    <main className="case-page">
      <section className="section-frame case-hero">
        <div className="case-hero-copy"><span className="eyebrow">{project.eyebrow}</span><h1>{project.title}: site de marca com narrativa visual e experiência de produto.</h1><p>{project.description}</p><div className="hero-actions"><a className="btn btn-primary coin" href="#contato-form">Quero algo parecido <ArrowUpRight size={16} /></a><a className="btn btn-secondary" href="/portfolio">Voltar ao portfólio <ArrowUpRight size={16} /></a></div><div className="case-stack">{project.stack.map((tech) => <span key={tech}>{tech}</span>)}</div></div>
      ...
      <ContactForm page="Case Beggin" context="criar uma experiência digital premium para marca, produto, narrativa visual, WordPress, performance e evolução" title="Quer transformar seu produto em uma experiência digital premium?" description="Use este bloco para iniciar um diagnóstico contextualizado sobre marca, produto, front-end e performance." />
      <section className="final-cta"><div><span className="eyebrow">Marca e produto</span><h2>Quer transformar seu produto em uma experiência digital premium?</h2><p>A Tehkné estrutura páginas de produto, sites de marca, front-end, performance e evolução para operações comerciais.</p></div><div className="hero-actions"><a className="btn btn-primary coin" href="#contato-form">Solicitar diagnóstico <ArrowUpRight size={16} /></a><a className="btn btn-secondary" href="/portfolio">Voltar ao portfólio <ArrowUpRight size={16} /></a></div></section>
    </main>
  );
}
