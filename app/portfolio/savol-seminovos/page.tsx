import { ArrowUpRight, Car, Database, Filter, Globe2, MapPin, Search, ShieldCheck, Sparkles } from 'lucide-react';
  ...
  return (
    <main className="site-shell case-page">
      ...
      <section className="section-frame case-hero">
        <div className="case-hero-copy"><span className="eyebrow">{project.eyebrow}</span><h1>{project.title}: catálogo automotivo com filtros e operação comercial.</h1><p>{project.description}</p><div className="hero-actions"><a className="btn btn-primary coin" href="/#contato">Quero algo parecido <ArrowUpRight size={16} /></a><a className="btn btn-secondary" href="/portfolio">Voltar ao portfólio <ArrowUpRight size={16} /></a></div><div className="case-stack">{project.stack.map((tech) => <span key={tech}>{tech}</span>)}</div></div>
      ...
      <section className="final-cta"><div><span className="eyebrow">Catálogos e portais</span><h2>Precisa transformar dados e produtos em uma experiência comercial?</h2><p>A Tehkné estrutura catálogos, filtros, páginas dinâmicas e manutenção para operações que vendem com conteúdo e tecnologia.</p></div><div className="hero-actions"><a className="btn btn-primary coin" href="mailto:contato@tehkne.com.br">Solicitar diagnóstico <ArrowUpRight size={16} /></a><a className="btn btn-secondary" href="/portfolio">Voltar ao portfólio <ArrowUpRight size={16} /></a></div></section>
    </main>
  );
}
