import { ArrowUpRight, CalendarDays, FileText, Globe2, HeartPulse, ShieldCheck, Sparkles, Users } from 'lucide-react';
  ...
  return (
    <main className="site-shell case-page">
      ...
      <section className="section-frame case-hero">
        <div className="case-hero-copy"><span className="eyebrow">{project.eyebrow}</span><h1>{project.title}: presença digital clara para vacinação, cuidado e expansão.</h1><p>{project.description}</p><div className="hero-actions"><a className="btn btn-primary coin" href="/#contato">Quero algo parecido <ArrowUpRight size={16} /></a><a className="btn btn-secondary" href="/portfolio">Voltar ao portfólio <ArrowUpRight size={16} /></a></div><div className="case-stack">{project.stack.map((tech) => <span key={tech}>{tech}</span>)}</div></div>
